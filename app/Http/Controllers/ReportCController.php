	...
	///	  $user->id);
		 // dd($user->id);
			...
			//	->where('cases.status','=',1)
                ...
                ->select(\DB::raw("cases.id ,
				cases_statuses.name as status,
				cases.description,cases.location,
				cases.gps_lat ,cases.gps_lng ,
				cases.actiontaken , cases.nuberof_person_involve ,cases.name_person_involve,cases.surname_person_involve ,cases.phone_person_involve,cases.email_person_involve,cases.incident_date_time,cases.img_url,
				categories.name as category,
               ...
               ->groupBy('cases.created_at')->get();
                ...
                $response["error"]   = FALSE;
                $response["reports"] = $myReports;
				$response["referecase"] = $referecase;
				$response["allocatereports"] = $allocatecase;
